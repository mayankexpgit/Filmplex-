...
import { z } from 'zod';
import { fetchMovieDetailsFromTrakt } from '@/services/traktService';
...
const getMovieDetailsFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    let traktData;
    try {
      // Step 2: Get factual data from Trakt.tv API using the corrected title
      traktData = await fetchMovieDetailsFromTrakt(correctedTitle, input.year);
    } catch (error: any) {
    ...
    const creativeInput: MovieDetailsOutput = {
      ...
      tags: [],
      synopsis: traktData.synopsis, // Pass original synopsis to AI for refinement
      description: '',
      cardInfoText: '',
    };
