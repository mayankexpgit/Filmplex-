...
function CommentsSection({ seriesId }: { seriesId: string }) {
    ...
    useEffect(() => {
        if (seriesId) {
            fetchCommentsForMovie(seriesId);
        }
        // Clear comments when series changes
        return () => setComments([]);
    }, [seriesId, setComments]);
    ...
    const handleCommentSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await submitComment(seriesId, userName, newComment);
            setNewComment("");
...
export default function SeriesDetailPage() {
  ...
  useEffect(() => {
    if (isInitialized && slug) {
      const foundSeries = allMovies.find(m => createSlug(m.title, m.year) === slug && m.contentType === 'series');
      setSeries(foundSeries);
    }
  }, [isInitialized, allMovies, slug]);

  if (!isInitialized || !series) {
    return <SeriesPageLoader />;
  }

  const handleDownloadClick = () => {
    if(series?.id) {
        recordDownload(series.id);
    }
  }

  const handleReactionClick = (reaction: keyof Reactions) => {
    if (hasReacted || !series.id) return;
    submitReaction(series.id, reaction);
    setHasReacted(true);
  }
    ...
    <div className="bg-background min-h-screen text-foreground">
      ...
      <main className="container mx-auto py-8 md:py-12">
        <div className="max-w-4xl mx-auto flex flex-col items-center">
          ...
          {series.cardInfoText && (
            <p className="text-2xl font-bold text-center mb-2 text-foreground whitespace-pre-line">{series.cardInfoText}</p>
          )}
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-center mb-6">{series.title} ({series.year})</h1>
          
          <div className="w-full grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
            <InfoItem icon={Star} label="TMDb Rating" value={series.imdbRating ? `${series.imdbRating}/10` : 'N/A'} />
            <InfoItem icon={LayoutGrid} label="Genre" value={series.genre} />
            <InfoItem icon={Users} label="Stars" value={series.stars} />
            <InfoItem icon={Video} label="Creator" value={series.creator} />
            <InfoItem icon={CalendarDays} label="Release Date" value={formattedReleaseDate} />
            <InfoItem icon={Globe} label="Country" value={series.country} />
            <InfoItem icon={Languages} label="Language" value={series.language} />
            <InfoItem icon={BadgeCheck} label="Quality" value={series.quality} />
            <InfoItem icon={ListVideo} label="Total Episodes" value={series.numberOfEpisodes} />
          </div>
          ...
          {series.screenshots && series.screenshots.length > 0 && (
            <section className="w-full mb-8">
              ...
              <div className="flex flex-col items-center">
                {series.screenshots.map((src, index) => (
                  <div key={index} className="relative w-full max-w-2xl overflow-hidden rounded-lg">
                    <Image
                      src={src}
                      alt={`Screenshot ${index + 1} for ${series.title}`}
                      width={1280}
                      height={720}
                      className="object-contain w-full h-auto"
                      data-ai-hint="series screenshot"
                    />
                  </div>
                ))}
              </div>
          ...
          {series.episodes && series.episodes.length > 0 && (
            <section className="w-full mb-8">
                ...
                <div className="space-y-6 w-full max-w-2xl mx-auto">
                    {series.episodes.map((ep, epIndex) => (
          ...
          {series.seasonDownloadLinks && series.seasonDownloadLinks.length > 0 && (
             <section className="w-full mb-12">
              <h2 className="text-2xl font-bold mb-4 text-center">Download Full Season</h2>
              <div className="space-y-3 flex flex-col items-center">
                {series.seasonDownloadLinks.map((link, index) => (
          ...
          {series.synopsis && (
            <section className="mb-8 w-full">
              ...
              <div 
                ...
                dangerouslySetInnerHTML={{ __html: series.synopsis.replace(/\n/g, '<br />') }}
              />
            </section>
          )}
