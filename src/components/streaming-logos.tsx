...
const StreamingLogos = () => {
  const logos = [
    ...
    { name: 'Crunchyroll', url: 'https://upload.wikimedia.org/wikipedia/commons/thumb/0/08/Crunchyroll_logo.svg/2560px-Crunchyroll_logo.svg.png' },
      ...
      <div className="flex justify-center items-center gap-6 md:gap-8 flex-wrap">
        {logos.map((logo) => (
          <div key={logo.name} className={`transition-all duration-300 ease-in-out transform hover:scale-105 relative ${
              logo.name === 'Prime Video' ? 'h-10 w-28 sm:h-12 sm:w-36' : 'h-8 w-24 sm:h-10 sm:w-32'
            }`}>
            <Image
              src={logo.url}
              alt={`${logo.name} logo`}
